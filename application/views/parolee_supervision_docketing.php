...
<body>
                    ...
                    <div class="col-md-12">
                        <div class="card">
                            <div class="card-header" id="pager">
                                ...
                                <a href="parolee_supervision_create"> <button class="btn btn-sm btn-success float-right pr_sup_add" type="submit"><i class="fa fa-plus-circle"></i> Add Docket</button> </a>
                            </div>
                            <div class="card-body">
    ...
    ( function ( $ ) {
        ...
        var __table = function(){
            ...
            __executeExternalGet('http://localhost:8000/docketbook/list/SC_PR_SUP/'+$.cookie("field_office_id")).done(function (result) {
                console.log("==========")
                console.log(result)
                console.log("==========")
                if (result.status != "ERROR") {
                    result.response.forEach(function(data){
                        $('.table_body').append("<tr>"+
                            ...
                            "<td>"+data.status+"</td>"+
                            "<td align='center' class='actions'> <button class='btn btn-sm btn-primary btn_update pr_sup_update' type='submit' data-docket='"+data.docketNumber+"'><i class='fa fa-refresh'></i> Update</button> <button class='btn btn-sm btn-danger btn_remove pr_sup_remove' type='submit' data-toggle='modal' data-target='#removeModal' data-docket='"+data.docketNumber+"' data-oi='"+data.fieldOfficeId+"'><i class='fa fa-remove'></i> Remove</button>")
                    });
                    ...
                    $(".btn_update").unbind("click").on("click", function(){
                        var docket_number = $(this).data("docket");
                        window.location.href = 'http://localhost/pis/parolee_supervision_update?docket_number='+docket_number;
                    })
