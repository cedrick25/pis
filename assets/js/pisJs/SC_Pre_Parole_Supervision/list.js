(function ($) {
    ...
    function bindRowActionsOnce() {
        $(document).off('click.scPprSup', '.btn_update').on('click.scPprSup', '.btn_update', function () {
            ...
            window.location.href = joinApiUrl(api, 'pis/parole-pardon-supervision-update?docket_number=' + encodeURIComponent(docket_number) + '&officeId=' + encodeURIComponent(officeId));
        });
        $(document).off('click.scPprSup', '.btn_view').on('click.scPprSup', '.btn_view', function () {
            var docket_number = $(this).data('docket');
            var officeId = $(this).data('oi') || $.cookie('field_office_id');
            window.location.href = joinApiUrl(api, 'pis/parole-pardon-supervision-view?docket_number=' + encodeURIComponent(docket_number) + '&officeId=' + encodeURIComponent(officeId));
        });
        $(document).off('click.scPprSup', '.btn_attachments').on('click.scPprSup', '.btn_attachments', function () {
            var docket_number = $(this).data('docket');
            var officeId = $(this).data('oi') || $.cookie('field_office_id');
            window.location.href = joinApiUrl(api, 'pis/parole-pardon-supervision-upload?docket_number=' + encodeURIComponent(docket_number) + '&officeId=' + encodeURIComponent(officeId));
        });
        ...
        return [
            ...
            { data: 'fieldOfficeName' },
            {
                ...
                render: function (data) {
                    ...
                    return (
                        '<button class="btn btn-sm btn-primary btn_view " data-permission="can_view_docket_pre_parole_supervision" style="display:none;" type="button" data-docket="' + dn + '" data-oi="' + oi + '"><i class="fa fa-eye"></i> View</button> ' +
                        '<button class="btn btn-sm btn-primary btn_update " data-permission="can_edit_docket_pre_parole_supervision" style="display:none;" type="button" data-docket="' + dn + '" data-oi="' + oi + '"><i class="fa fa-edit"></i> Update</button> ' +
                        '<button class="btn btn-sm btn-primary btn_attachments " data-permission="can_attachments_docket_pre_parole_supervision" style="display:none;" type="button" data-docket="' + dn + '" data-oi="' + oi + '"><i class="fa fa-paperclip"></i> Attachments</button> ' +
                        '<button type="button" class="btn btn-sm btn-danger btn_remove " data-permission="can_delete_docket_pre_parole_supervision" style="display:none;" data-toggle="modal" data-target="#removeModal" data-docket="' + dn + '" data-oi="' + oi + '" aria-label="Remove docket ' + dn + '" title="Remove"><i class="fa fa-remove" aria-hidden="true"></i> Remove</button>'
                    );
                }
            }
        ];
