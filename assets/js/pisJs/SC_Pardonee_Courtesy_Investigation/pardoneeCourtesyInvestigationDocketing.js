    ( function ( $ ) {
        ...
        function buttonFunctionality(){
            ...
            $(".btn_update").unbind("click").on("click", function(){
                var docket_number = $(this).data("docket");
                window.location.href = api+'/pis/pardonee_courtesy_investigation_update?docket_number='+docket_number;
            })

            $(".btn_view").unbind("click").on("click", function(){
                var docket_number = $(this).data("docket");
                window.location.href = api+'/pis/pardonee_courtesy_investigation_view?docket_number='+docket_number;
            })
        }
        ...
        //                 const apiUrl = api+"8000/docketbook?page="+page+"&size="+size+"&type=SC_PD_CINV&officeId="+$.cookie('field_office_id');
        //                 $.ajax({
        //                     url: apiUrl,
        //                     method: 'GET',
        //                     dataType: 'json',
        ...
        function tableColumns() {
            return [
                ...
                },
                {
                    "data": null,
                    render: function(data, type, row) {
                        return "<button class='btn btn-sm btn-primary btn_update pd_cinv_update' style='display:none;' type='submit' data-docket='"+data.docketNumber+"'><i class='fa fa-edit'></i> Update</button> <button class='btn btn-sm btn-primary btn_view pd_cinv_view' style='display:none;' type='submit' data-docket='"+data.docketNumber+"'><i class='fa fa-eye'></i> View</button> <button class='btn btn-sm btn-danger btn_remove pd_cinv_remove' style='display:none;' type='submit' data-toggle='modal' data-target='#removeModal' data-docket='"+data.docketNumber+"' data-oi='"+data.fieldOfficeId+"'><i class='fa fa-remove'></i> Remove</button>";
                    }
                }
            ]
        }
