        ...
        function buttonFunctionality() {
            ...
            $(".btn_grant").unbind("click").on("click", function(){
                ...
                __executeExternalGet('8088/permission/list').done(function (result) {
                    ...
                    if (result.status != "ERROR" && window.PisPermissionTree) {
                        const permissions = window.PisPermissionTree.buildPermissionTree(result);

                        function renderPermissions(container, items, depth) {
                          depth = depth || 0;
                          items.forEach(item => {
                            const wrapper = document.createElement("div");
                            const hasChildren = item.subcategories && item.subcategories.length > 0;
                            const levelClass = depth === 0 ? 'is-module' : (hasChildren ? 'is-submodule' : 'is-action');
                            wrapper.classList.add("permission-item", "col-md-12", "row", levelClass);
                            ...
                            if (hasChildren) {
                              ...
                              renderPermissions(subContainer, item.subcategories, depth + 1);
            ...
            $(".btn_grant_update").unbind("click").on("click", function(){
                console.log("clicked button grant update")
                var data_id = $(this).data("id");
                console.log(data_id)

                __executeExternalGet('8088/permission/list').done(function (resultList) {
                    ...
                    if (!resultList || resultList.status == "ERROR" || !window.PisPermissionTree) {
                        console.log("failed fetching permission list");
                        return;
                    }

                    const granted = {};
                    if (result_rp && result_rp.status != "ERROR" && Array.isArray(result_rp.permissionList)) {
                        result_rp.permissionList.forEach(function (p) {
                            granted[String(p.id)] = !!p.value;
                        });
                    }

                    const permissions = window.PisPermissionTree.buildPermissionTree(resultList);

                    function renderPermissionsUpdate(container, items, depth) {
                      depth = depth || 0;
                      items.forEach(item => {
                        const wrapper = document.createElement("div");
                        const hasChildren = item.subcategories && item.subcategories.length > 0;
                        const levelClass = depth === 0 ? 'is-module' : (hasChildren ? 'is-submodule' : 'is-action');
                        wrapper.classList.add("permission-item", "col-md-12", "row", levelClass);
                        const isChecked = granted[String(item.id)] === true ? "checked" : "";
                        ...
                        wrapper.innerHTML = `
                          <div class="permission-row">
                            <label class="switch">
                              <input type="checkbox" class="permission-checkbox" name="type" value="${item.id}"
                                     data-name="${item.label}" data-action="${item.action}"
                                     data-detail="${item.permission}" ${isChecked}>
                              <span class="slider round"></span>
                            </label>
                            <span class="permission-label">${item.label}</span>
                          </div>
                        ...
                        if (hasChildren) {
                          const subContainer = document.createElement("div");
                          subContainer.classList.add("sub-permission", "col-md-12", "row");
                          renderPermissionsUpdate(subContainer, item.subcategories, depth + 1);
                          wrapper.appendChild(subContainer);
                    ...
                    const updateContainer = document.querySelector('.permission_list_update');
                    renderPermissionsUpdate(updateContainer, permissions);
                    });
                });
                ...
                $(".btn_grant_confirm_update").unbind("click").on("click", function(){
                    console.log('clicked btn grant confirm')
                    const sentence = [];
                    $(".permission_list_update input:checkbox[name=type]:checked").each(function(){
                        var list = {
                          "id": $(this).val(),
                          "value": true,
                          "name": $(this).data("name"),
                    ...
                    $(".permission_list_update input:checkbox[name=type]:not(:checked)").each(function(){
                        var list = {
                          "id": $(this).val(),
                          "value": false,
                          "name": $(this).data("name"),
