(function ($) {
    ...
    $(document).on('click.supAttDel', '.table_head .btn-delete', function () {
    ...
    function bindUploadConfirm(officeId, createdByName, docketRow) {
        $('.btn-confirm').off('click.supAttUp').on('click.supAttUp', function () {
            hideAttachmentsError();
    ...
    function startPage() {
        ...
        if (!docket_number) {
            setPageLoader(false);
            showAttachmentsError('This page is missing a docket number. Open attachments from the Supervision Docket list.');
            return;
        }
        if (!fi) {
            setPageLoader(false);
        ...
        __executeExternalGet('8088/user/' + userUuid).done(function (userProfile) {
            ...
            __executeExternalGet('8000/docketbook/' + encodeURIComponent(docket_number) + '/' + encodeURIComponent(fi)).done(function (apiResult) {
                ...
                loadTable('supervision', docketRow.docketNumber, docketOfficeId);
    ...
    $(function () {
        $('.cmisTable').on('change.supCmis', function () {
            hideAttachmentsError();
