(function ($) {
    ...
    function setPageLoader(busy) {
        var $el = $('#csup_attachments_loader');
        if (!$el.length) {
            return;
        }
        if (busy) {
    ...
    var fileListType = GetURLParameter('type');
    if (fileListType !== undefined && fileListType !== null) {
        fileListType = String(fileListType).trim();
    }
    if (!fileListType) {
    ...
    function populateTypeOptions(cmisVal) {
        ...
        if (cmisVal === 'F5T13RR') {
            ...
            $t.append('<option value="none" disabled selected>Select</option>');
            showAttachmentsError('Invalid Kind selection. Choose Referrals Received or Referrals Terminated.');
        }
        $t.trigger('change');
    }
    ...
    $(document).on('click.csupAttDel', '.table_head .btn-delete', function () {
    ...
    function startPage() {
        ...
        if (!docket_number) {
            setPageLoader(false);
            showAttachmentsError('This page is missing a docket number. Open attachments from the Courtesy Supervision list.');
            return;
        }
        if (!fi) {
            setPageLoader(false);
    ...
    $(function () {
        $('.cmisTable').on('change.csupCmis', function () {
            hideAttachmentsError();
